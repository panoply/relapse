...
function $folds(scope, event) {
  ...
  return (fold) => {
    ...
    fold.close = (index) => {
      ...
      if (config.multiple) {
        if (!config.persist || config.persist && scope.count > 1)
          $collapse(focus);
      } else {
...
var $defaults = (options, attrs) => {
  ...
  config.classes = /* @__PURE__ */ Object.create(null);
  config.persist = true;
...
function relapse(selector, options) {
  const el = typeof selector === "string" ? document.body.querySelector(selector) : selector;
  if (!el)
    return;
